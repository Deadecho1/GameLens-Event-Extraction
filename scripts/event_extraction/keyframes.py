...
class KeyframePolicy(Protocol):
    def pick(self, interval: Interval) -> Tuple[List[int], Dict[str, Any]]:
        ...
...
class KeyframePicker:
    ...
    def pick(self, interval: Interval) -> Tuple[List[int], Dict[str, Any]]:
        policy = self._policies.get(interval.label, self._default)
        return policy.pick(interval)
